...
# --- YUKLASH FUNKSIYASI (80MB Limit va Sifatni avtomatik pasaytirish) ---
async def download_media(url, mode, quality="720"):
    if not os.path.exists('downloads'): os.makedirs('downloads')
    
    # Sifat zanjiri: Agar 720p katta bo'lsa -> 480p -> 360p
    quality_chain = ["720", "480", "360"]
    start_index = quality_chain.index(quality) if quality in quality_chain else 0
    
    for q in quality_chain[start_index:]:
        ydl_opts = {
            'outtmpl': f'downloads/%(id)s.%(ext)s',
            'quiet': True,
            'noplaylist': True,
            'max_filesize': 80 * 1024 * 1024 # 80MB Limit
        }
        ...
        if mode == "mp3":
            ...
            ydl_opts['format'] = f'bestvideo[height<={q}]+bestaudio/best/best[height<={q}]'
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                path = ydl.prepare_filename(info).replace(".webm", ".mp4").replace(".m4a", ".mp3").replace(".mkv", ".mp4")
                return path
        except Exception as e:
            if "File is larger than" in str(e) and q != "360":
                continue # Keyingi pastroq sifatni sinab ko'radi
            else:
                return "error"
    return "error"

# --- MAJBURIY OBUNA (Aqlli: Kanallar bo'sh bo'lsa o'tkazib yuboradi) ---
async def check_sub(user_id):
    channels = await channels_col.find().to_list(length=10)
    if not channels:
        return True # Kanallar hali qo'shilmagan bo'lsa, yuklab beraveradi
...
async def start(message: types.Message):
    ...
    start_text = (
        "👋 **Assalomu alaykum!**\n\n"
        "🤖 Men YouTube, Instagram, TikTok va boshqa tarmoqlardan video yuklovchi botman.\n\n"
        "📌 **Imkoniyatlarim:**\n"
        "— Videolarni yuklash (Maksimal 80MB)\n"
        "— Musiqalarni yuklash (MP3)\n"
        "— Avtomatik sifatni sozlash (Hajm katta bo'lsa)\n\n"
...
async def handle_url(message: types.Message):
    if not await check_sub(message.from_id):
        ...
        return await message.answer("⚠️ Botdan foydalanish uchun quyidagi kanallarga obuna bo'ling!", reply_markup=kb)
    ...
    await message.answer("Hajm maksimal 80MB. Tanlang:", reply_markup=kb)
...
async def dl_callback(call: CallbackQuery):
    ...
    msg = await call.message.edit_text("⏳ Yuklanmoqda, iltimos kuting...")
    
    path = await download_media(url, q, q if q.isdigit() else "720")
    
    if path == "error":
        await call.message.answer("❌ Bu videoni yuklab bo'lmaydi. (Hajmi 80MB dan katta yoki havola noto'g'ri). Iltimos, to'g'ri video havolasini yuboring.")
        await msg.delete()
        return

    try:
        with open(path, 'rb') as f:
            if q == "mp3": await call.message.answer_audio(f, caption="@Allsaver")
            else: await call.message.answer_video(f, caption="@Allsaver")
        await msg.delete()
    except Exception:
        await call.message.answer("❌ Telegramga yuborishda xatolik yuz berdi.")
    finally:
        if path and os.path.exists(path) and path != "error": 
            os.remove(path)

# --- REKLAMA VA KANAL QO'SHISH ---
...
async def ch_start(call: CallbackQuery):
    await call.message.answer("Format: `-100xxx https://t.me/url` (ID va Link orasida probel bo'lsin)")
...
async def ch_save(message: types.Message, state: FSMContext):
    ...
    if len(d) < 2:
        return await message.answer("Xato format. Qaytadan urinib ko'ring.")
...
async def a_stat(call: CallbackQuery):
    ...
    await call.message.answer(f"Bot foydalanuvchilari: {count} ta")
